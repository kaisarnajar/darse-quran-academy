...
export function BankDetailsCard({ bank, paymentRef, amountLabel }: BankDetailsCardProps) {
  const rows = [
    ...
    { label: "Reference (include in remarks)", value: paymentRef, mono: true },
  ];
      ...
      <dl className="mt-4 space-y-3 text-sm">
        {rows.map((row) => (
          <div key={row.label} className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
            ...
            <dd className={`text-right font-medium text-foreground sm:text-left ${row.mono ? "font-mono text-xs sm:text-sm" : ""}`}>
              {row.value}
            </dd>
          </div>
        ))}
