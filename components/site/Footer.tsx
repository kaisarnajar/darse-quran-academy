    ...
    <footer className="mt-auto cta-gradient text-white">
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-12">
        <div className="grid grid-cols-1 gap-8 text-center sm:grid-cols-2 sm:text-left lg:grid-cols-3">
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-cyan-300">Quick Links</p>
            <ul className="mt-3 space-y-2">
              {footerLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="inline-flex min-h-10 items-center text-sm text-blue-100 transition-colors hover:text-white"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        <p className="mt-8 border-t border-white/15 pt-6 text-center text-xs text-blue-200/80">
