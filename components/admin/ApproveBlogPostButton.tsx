...
export function ApproveBlogPostButton({ postId }: { postId: string }) {
  ...
  function handleApprove() {
    if (!window.confirm("Approve this blog post and publish it on the public blog?")) return;

    startTransition(async () => {
      try {
        const result = await approveBlogPost(postId, getReturnTo());
        if (result?.error) {
        ...
        window.alert("Could not approve blog post. Please try again.");
      }
    });
  }
