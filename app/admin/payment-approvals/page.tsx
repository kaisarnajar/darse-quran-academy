...
import { getPendingMonthlyPayments } from "@/lib/monthly-payments";
...
export default async function AdminPaymentApprovalsPage({
  ...
  const [pendingPayments, courses] = await Promise.all([
    ...
    <div>
      ...
      {params.confirmed === "1" && (
        <p className="mt-4 rounded-md bg-violet-50 px-4 py-3 text-sm text-violet-800">
          Payment approved and recorded on the student&apos;s account.
        </p>
      )}
      {params.declined === "1" && (
